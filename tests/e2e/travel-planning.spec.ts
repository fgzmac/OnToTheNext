...
let firstDay:string,secondDay:string,activityId:string,sourceId:string;
...
test.beforeEach(async()=>{
 ...
 ok(await createPlanningBlock({tripId,dayId:secondDay,type:"TRANSPORTATION",transportationMode:"TRAIN",startMinute:15,durationMinutes:120}));
});
...
for(const [device,width,height] of [["desktop",1280,900],["phone",390,844]] as const){
 test(device+" manual leave-by, reviewed context, basis choice, source conflict and midnight",async({page},testInfo)=>{
  ...
  const card=page.getByRole("article",{name:title,exact:true}),editor=page.getByRole("dialog").locator(".travel-planning");
  ...
  const open=async()=>{await expect(page).toHaveURL(/\/itinerary(?:[?#]|$)/);if(!new URL(page.url()).hash) { const url=new URL(page.url()); url.hash="item-"+activityId; await page.goto(url.toString()); } await expect(page.getByRole("dialog")).toBeVisible(); if(await editor.getAttribute("open")===null)await editor.getByText("Travel planning",{exact:true}).click();};
  ...
  await closeDetails(page); await nav.getByRole("link",{name:"Home",exact:true}).click();await expect(page.getByLabel("Trip day",{exact:true})).toHaveValue(firstDay);
  ...
  await closeDetails(page);await page.getByRole("button",{name:"Open Train transportation",exact:true}).click();const trainEditor=page.getByRole("dialog").locator(".travel-planning");await trainEditor.getByText("Travel planning",{exact:true}).click();
  ...
  await closeDetails(page); await nav.getByRole("link",{name:"Home",exact:true}).click();await page.getByLabel("Trip day",{exact:true}).selectOption(secondDay);await page.getByRole("button",{name:"Show day",exact:true}).click();await expect(card).toContainText("From planned stop: Hotel / Rest");await expect(card).toContainText("09:25");await expect(page.getByText("Estimated leave-by: 2030-04-01 · 23:35",{exact:true})).toBeVisible();await capture("midnight-and-conflict");
  ...
  await closeDetails(page); await nav.getByRole("link",{name:"Home",exact:true}).click();await expect(page).toHaveURL("/trips/"+tripId+"?view=day&day="+secondDay);await expect(page.getByLabel("Trip day",{exact:true})).toHaveValue(secondDay);await page.reload();await expect(page.getByLabel("Trip day",{exact:true})).toHaveValue(secondDay);await expect(card).toContainText("Travel estimate not entered.");await expect(nav.getByRole("link")).toHaveText(["Home","Itinerary","Discover"]);
 });
}
