...
export function RecommendationPhoto({ photo, name }: { photo?: ExperiencePhoto | null; name: string }) {
  ...
  if (!photo || failed) return <div className="idea-photo-fallback" role="img" aria-label={"Photo unavailable for " + name}>{failed ? "Photo could not load" : "Photo unavailable"}</div>;
  // Remote dimensions are presentation defaults, not claimed source-image metadata.
  return <figure className="idea-photo" data-framing="full">
