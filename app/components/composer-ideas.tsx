...
export function RecommendationSources({ item }: { item: RecommendationCardData }) {
  return <details className="idea-sources"><summary>Details and sources</summary>
    {item.evidence.map(e => <div key={e.id}><p>{e.factualText}</p>
      {e.sourceUrl ? <a href={e.sourceUrl} target="_blank" rel="noopener noreferrer">Official source</a> : <p>{e.sourceName}</p>}
      <p className="muted">{e.sourceKind === "OFFICIAL_CURATED" ? "Observed " : "Record dated "}{e.retrievedAt.slice(0, 10)} · {e.sourceKind === "DEVELOPMENT_FIXTURE" ? "Synthetic test data" : "Not live availability"}</p></div>)}
    {!item.evidence.length ? <p>No source details available.</p> : null}
    {item.durationMinutes !== null ? <p>Duration is a planning estimate, not provider-confirmed timing.</p> : null}
  </details>;
}
export function ComposerIdeas({ tripId, segmentId, dayId, dayNumber, initial, error: initialError, revision }: {
  ...
  const refresh = async (page: number, more = false) => {
    const result = await composerBatch(tripId, segmentId, page, more);
    if (!result.ok) { setError(result.error); return; }
    setBatch(result.data);
  };
  useEffect(() => { let active = true; composerBatch(tripId, segmentId, batch?.page ?? 0).then(result => { if (active) { if (result.ok) setBatch(result.data); else setError(result.error); } }).catch(() => { if (active) setError("Ideas could not be refreshed. Try again."); }); return () => { active = false; }; }, [revision, tripId, segmentId, batch?.page]);
  ...
  return <section className="composer-ideas stack" aria-label="Ideas for selected day">
    ...
    {cards.map(item => <article key={item.id} className={"idea-card" + (item.scheduledDay ? " idea-scheduled" : "")} aria-labelledby={"recommendation-" + item.id}>
      ...
      <RecommendationSources item={item} /></>}
    </article>)}
