...
export function GoogleDetailsView({state,tab,setTab,confirm,reviews,refresh,photoFailed,fallbackPhoto}:{state:DetailState;tab:"overview"|"reviews";setTab:(tab:"overview"|"reviews")=>void;confirm:(token:string)=>void;reviews:()=>void;refresh:()=>void;photoFailed:()=>void;fallbackPhoto?:ReactNode}){
 ...
 const copy=state.warning??(state.status==="disabled"?"Place details aren’t connected.":state.status==="loading"?"Finding place details…":state.status==="unavailable"?"We couldn’t verify this location.":state.status==="budget_exhausted"?"Place details allowance reached.":state.status==="expired"?"Place details expired. Refresh to check again.":state.status==="ambiguous"?"Choose the location that matches this activity.":state.status==="needs_confirmation"?"Check this location before using its details.":"");
 return <section className="google-context stack" aria-label="Activity place details">
  ...
  <div role="tablist" aria-label="Activity information" className="activity-tabs" onKeyDown={e=>{if(e.key==="ArrowLeft"||e.key==="ArrowRight"){e.preventDefault();const next=tab==="overview"?"reviews":"overview";if(next==="reviews"&&!p)return;setTab(next);if(next==="reviews")reviews();document.getElementById(id+"-"+next)?.focus();}}}>
   {(["overview","reviews"] as const).map(t=><button key={t} type="button" role="tab" id={id+"-"+t} aria-controls={id+"-"+t+"-panel"} aria-selected={tab===t} tabIndex={tab===t?0:-1} disabled={t==="reviews"&&!p} onClick={()=>{setTab(t);if(t==="reviews")reviews();}}>{t==="overview"?"Overview":"Reviews"}</button>)}
  </div>
  {photo?<figure className="activity-google-photo">{/* eslint-disable-next-line @next/next/no-img-element */}
  ...
  {state.sections.photo==="loading"?<p role="status">Loading photo…</p>:state.sections.photo==="unavailable"?<p>Photo unavailable.</p>:state.sections.photo==="budget_exhausted"?<p>Photo allowance reached.</p>:null}
  {p?<div className="google-provider-content">
   <p>{p.formattedAddress}</p>{p.rating!==undefined?<p className="activity-rating">{p.rating} / 5{p.userRatingCount!==undefined?" · "+p.userRatingCount+" ratings":""}</p>:null}
   <Link url={p.googleMapsUri}>View on Google Maps</Link><Attribution place={p}/>
  </div>:null}
  <div role="tabpanel" id={id+"-overview-panel"} aria-labelledby={id+"-overview"} hidden={tab!=="overview"}>
...
export function GoogleContext({session,fallbackPhoto}:{session:DetailSession;fallbackPhoto?:ReactNode}){
 const state=useSyncExternalStore(session.subscribe,session.snapshot,session.snapshot);
 const [tab,setTab]=useState<"overview"|"reviews">("overview");
 return <GoogleDetailsView state={state} tab={tab} setTab={setTab} confirm={t=>void session.confirm(t)} reviews={()=>void session.reviews()} refresh={()=>void session.refresh()} photoFailed={()=>session.photoFailed()} fallbackPhoto={fallbackPhoto}/>;
}
