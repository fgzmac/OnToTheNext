...
export async function enrichGoogle(input:GoogleInput,deps:GoogleDependencies={}):Promise<Enrichment> {
  ...
  if(!config) return {message:"Google enrichment is disabled. Your recommendations and Add remain available."};
  if(!["identity","context","reviews","photo","confirm"].includes(input.purpose) || !/^[a-zA-Z0-9-]{16,80}$/.test(input.requestId)) throw new Error("INVALID_REQUEST");
  ...
  if(!app || metadata.kind==="EVENT" || metadata.unavailable || recommendation.place.research) return {message:"This slice enriches independently curated Japan places only. Google venue data cannot verify an event occurrence."};
  ...
  if(input.purpose==="confirm") {
    ...
    await db.$transaction(async tx=>{
      ...
      await tx.googlePlaceReference.upsert({where:{appPlaceId:app.id},create:{appPlaceId:app.id,googlePlaceId:t.id,identityHash:identity,reviewedAt:now()},update:{googlePlaceId:t.id,identityHash:identity,reviewedAt:now()}});
    });
    return {matched:true,message:"Place ID linked. Independent description, saved plan and bookings are unchanged. Request current context when needed."};
  }
  const ref=await db.googlePlaceReference.findUnique({where:{appPlaceId:app.id}});
  if(input.purpose!=="identity" && (!ref || ref.identityHash!==identity)) return {message:"Find and review an exact Google match first."};
  ...
  try {
    ...
    if(input.purpose==="identity") {
      ...
      result={candidates,message:count>1?"Ambiguous: multiple corroborated candidates. Review exact venue before linking; no match is selected automatically.":count===1?"Review this candidate before linking. Matching does not verify access, tickets or a walking route.":"No corroborated match. Independent recommendation and Add remain available."};
    } else {
      ...
      result={place:visible(p),matched:true,observedAt:now().toISOString(),message:"Google context for the linked place. Current/regular hours do not confirm future holiday access or availability."};
    ...
    return {message:"Google context is unavailable or the match changed. Recheck the match; your independent recommendation and Add remain available. Reserved cost is retained conservatively."};
  }
}
