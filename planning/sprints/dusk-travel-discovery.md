...
Final accessibility follow-up distinguishes serious location warnings from routine expiry, gives interactive borders at least 3:1 contrast against each surface, and checks keyboard focus visibility across the long-title/enlarged-text state. All 36 captures reran successfully after these corrections; minimum measured text contrast remains 5.74:1. Typecheck, scoped lint and 18 focused tests pass. The first published UI revision (7e09b70fb0f2159a995e10ed8188cfc78276bc9c) passed both CI runs, including 660 unit/integration cases, production build and 35 browser flows with no failures or retries reported. The final accessibility commit receives separate exact-head CI; its results and full SHA are recorded in draft PR7 and the owner handoff.
