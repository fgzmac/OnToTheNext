...
Subsequent CI-only diagnostics: attempt-1 push/PR runs at `6d3df24` (`35946451009` / `35946454302`, PR checkout `9497f8dc7288b6d93eb812ecbc75b1d2b30824f9`) passed volume checks but rejected the server-address comparison. A diagnostic-only commit `08a80cd` retained all guards; runs `35946578702` / `35946582103` (PR checkout `243b7c4eb5d1ed6adbe992783de8a4c96c3d30de`) printed the exact authorized database/user/public schema/5432 server, but PostgreSQL inet text included `/32` while Docker used a bare IPv4 address. All these runs stopped before migrations/tests. Use PostgreSQL `host(inet_server_addr())` for the exact Docker address comparison, preserving all identity, ownership, freshness and empty-schema checks. Only CI preflight/documentation changed; no application fix was required. Final runner results are reported in the PR/handoff rather than inferred from local validation.

## Google enrichment follow-on

The next scoped implementation is [Japan quality — Google enrichment](google-places-enrichment.md), disabled by default and verified only with synthetic transports. It reuses this branch's metadata, builder and durable accounting infrastructure. This does not activate Google/Brave/AI, complete broad discovery/event ingestion, or accept itinerary quality. The record preserves the later Japanese-source, Viator and feasibility/downtime slices.
