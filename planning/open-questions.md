...
**Next question to discuss:** Q-004 — monthly budget for running and testing the app, separate from the travel budget.
...
**Already answered:** The immediate goal is developing and testing for the Japan trip; the ultimate goal is commercial app-store or SaaS launch with subscription and ad revenue. The pilot audience is a couple: two travelers total, one organizer and one companion. For both hotels and transportation, the first version must organize existing bookings and recommend options before booking. Tokyo is a confirmed destination. Travel date endpoints and Tokyo-local arrival/departure times are supplied in chat, with only general timing context retained publicly. **November 10, 2026 is approved for the first complete test.** See D-006 through D-013 and resolved Q-002/Q-003/Q-007/Q-009 plus the timing portion of Q-004. Do not re-ask these choices, the year/testing target, or supplied travel values.

| ID | Question | Why it matters | Status |
| --- | --- | --- | --- |
| Q-004 | What monthly budget is available for running and testing the app? What remaining one-time cost, development-time, and maintenance constraints apply? | Helps assess scope and later service choices against the approved test target without assuming spending or mixing app costs with travel costs. | PARTIALLY ANSWERED — November 10, 2026 target approved (D-012), local travel timing supplied (D-013); next ask monthly app budget only, then remaining capacity questions one at a time. |
...
**Timing clarification:** The first complete test target is approved, not merely proposed. It is a software milestone, not a public-launch date or a guarantee that unestimated scope is feasible. The pilot uses the 2026 planning calendar and spans a month boundary. Arrival and departure clock times were supplied in Tokyo local time and remain private in the conversation. Those values do not establish airports, transfer durations, hotel nights, hotel-ready times, or full activity days. Tokyo is included; additional destinations remain open.

**Budget clarification:** Ask about monthly app running/testing costs, such as hosting and any paid data or AI services selected later. Do not assume AI, vendors, purchases, pricing, or a funded budget. One-time setup/development costs and personal development capacity remain separate follow-up topics.
...
| ID | Question | Status |
| --- | --- | --- |
| Q-201 | Which trip basics are required initially, and is date-free exploration supported? How are travel dates, destination-local arrival/departure times, and usable activity time distinguished? | OPEN for product behavior — the pilot's date endpoints and Tokyo-local clock times are already supplied privately; do not ask for them again |
| Q-202 | Can users choose a country/state, jump directly to a city, or plan multiple destinations? | OPEN — parked for Section 3; Tokyo is confirmed for the pilot, but a Tokyo-only trip is not |
...
| Q-209 | How do travelers add existing hotel/transport bookings, receive and choose recommendations, see them alongside activities, and review changes? Which criteria, entry methods, local times, buffers, costs, statuses, and booking handoffs are needed? | OPEN for detail — both support paths confirmed by D-010; no import, provider, transfer duration, or transaction mechanism selected |

## Section 4 — First-Release Scope and Success

| ID | Question | Status |
| --- | --- | --- |
| Q-301 | What additional destinations, actual local-day coverage, languages, devices, and broader trip/group limits must be supported beyond the confirmed pilot context? | PARTIALLY ANSWERED — Japan/Tokyo, two travelers, 2026 planning calendar, month-spanning travel context, and partial arrival/departure timing are supplied; additional cities and remaining coverage OPEN |
...
| Q-303 | Are multi-city travel, voting, advanced replanning, and in-app booking/payments excluded initially? Which multi-city and hotel/transport needs arise from the pilot? | OPEN — exclusions are proposals; naming Tokyo does not settle a single-city limit; organizing or recommending bookings is not the same as making reservations |
| Q-304 | Which completion, companion-usability, scheduling-quality, effort, and cost measures will be tracked? What establishes a complete test at the approved milestone? | OPEN — November 10, 2026 target approved; measurable acceptance conditions still to be defined |
| Q-305 | What does realistic scheduling promise, including accommodation, transportation, local-time meaning, partial days, and airport/transfer constraints? Which unknowns or manual checks must be disclosed? | OPEN — supplied flight times are not hotel-ready times or instructions for when to leave a hotel; exact buffers and guarantees not approved |
...
| Q-352 | What real-data samples and manually checked itineraries should be used for feasibility tests, including existing-booking organization and hotel/transport recommendations? | OPEN — Tokyo, a month-spanning trip, and partial arrival/departure days inform candidate tests; exact samples and tests remain OPEN. Use synthetic public test data, not private flight dates/times. |
| Q-353 | Which risks must be resolved before implementation, and what are their test/fallback criteria? | OPEN — parked for Section 5; do not assume reliable hotel availability, transport timetables, prices, reservation access, or feasibility within the approved target |
...
| Q-404 | Which logical concepts and persistent states are needed, including saved activities, scheduled occurrences, hotel stays, transportation legs, date/time meaning, and published versions? | OPEN — Section 6; these are candidate distinctions, not an approved schema |
...
| Q-506 | Has the owner explicitly approved design completion and the transition to implementation? | OPEN — implementation remains unauthorized in this workflow; accepting a test date did not approve this transition |
...
| R-004 | When will Codex implementation begin? | After the design phase and explicit transition approval; no implementation start authorized. | D-003 |
| Q-002 | What is the primary project objective and priority? | Develop and test for the Japan trip first; ultimately launch commercially through a mobile app store or as SaaS. Pilot and public launch are separate milestones. | D-006, D-007 — resolved 2026-09-19; travel context clarified subsequently |
...
| Q-004 (timing) | What is the first complete test target, including year, and has local arrival/departure timing been supplied? | November 10, 2026 is approved for the first complete pre-trip test. Travel endpoints and Tokyo-local times are supplied in chat and remain private. Budget/capacity portions of Q-004 remain active. | D-012, D-013 — timing recorded 2026-09-19 |
...
**Current next step:** Ask for the monthly app running/testing budget under Q-004, not the travel budget. Address remaining one-time costs and development/maintenance capacity afterward, one at a time. Do not re-ask the approved test target/year, supplied travel dates or clock times, Q-009, or the pilot group/count. Keep exact private travel details out of this public repository.
